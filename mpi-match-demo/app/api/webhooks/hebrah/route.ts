...
import { addQueueEvent } from '@/lib/webhook-store'

const ALLOWED = ['mpi.match.', 'mpi.link.']
...
export async function POST(request: Request) {
  ...
  if (!ALLOWED.some(prefix => eventType.startsWith(prefix))) {
    return NextResponse.json({ ignored: true }, { status: 202 })
  }

  const stored = addQueueEvent({ event: eventType, orgId, payload })
