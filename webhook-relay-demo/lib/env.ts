...
export function getDefaultPatientId() {
  return process.env.HEBRAH_DEFAULT_PATIENT_ID ?? 'pat_00000000_01'
}

export function getSidecarWritebackUrl() {
  return process.env.SIDECAR_WRITEBACK_URL ?? ''
}
...
export function getEnvStatus() {
  return {
    ...
    sidecarWritebackUrl: Boolean(process.env.SIDECAR_WRITEBACK_URL)
  }
}
