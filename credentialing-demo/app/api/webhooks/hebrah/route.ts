...
import { addCredentialingEvent } from '@/lib/webhook-store'
...
export async function POST(request: Request) {
  ...
  if (!eventType.startsWith('credentialing.')) {
    return NextResponse.json({ ignored: true }, { status: 202 })
  }

  const stored = addCredentialingEvent({ event: eventType, orgId, payload })
